...
metadata:
  ...
  duration: "1-2 分钟"
  changelog:
    - v1.4.0 (2026-06-11): 定位为短版本 — 产品界面回扣聚焦输入和报告，中间过程灵活处理
    - v1.3.0 (2026-06-11): 增加"回扣产品界面"指导 — 输入/处理/输出三个环节必须展示产品界面
    - v1.2.0 (2026-06-11): 强化实践导向 — 脚本必须包含解决方案部分，不能只诊断问题
...
撰写简洁高效的 Atypica usecase 脚本（1-2min 视频文案），覆盖产品痛点、解决方案、产品优势及适度 CTA。

> **核心目标：** 用 150-200 词讲好一个故事，用数据和原话说话，展示 Atypica 的独特价值。

> **版本定位：** 短版本（1-2min），产品界面回扣聚焦在**输入和报告**环节，中间过程可灵活呈现。  
> **长版本：** 需要详细的全流程产品演示？使用 `usecase-writer-demo`（3-5min）。
...
Usecase 的画面必须回扣到 **Atypica 产品本身**，让受众相信这些内容是产品生成的。

**短版本（1-2min）的产品界面回扣策略：**

**推荐回扣的环节：**

1. **输入场景** — 用户问题输入到对话框
2. **输出场景** — 完整报告 overview 或报告页面截图

**可灵活处理的环节：**

3. **处理场景** — 中间过程可用图形/动效替代产品界面

**标注示例：**
```
Their question was direct【产品界面：问题输入到对话框】: 
"How do we rebuild brand relevance with Gen Z?"

[中间过程：Persona 卡片、图表等视觉元素]

atypica delivered the full roadmap【产品界面：报告 overview】
in two weeks.
```

**检查清单（短版本）：**
- [ ] **是否有产品界面回扣画面？（至少在一个关键环节展示产品界面）**
- [ ] 核心内容（痛点、发现、解决方案）是否清晰呈现？
- [ ] 视觉元素（图形、动效、数据）是否丰富？
...
✅ 好的示例：
```
First, the perception gap: users trust Yili, but never share it. 
As one persona put it: "It's in my fridge, but not in my social feed."

Second, the execution trap: marketing feels forced, not native.
Another noted: "They're trying to be cool, but it feels like my dad learning slang."

Third, the distribution blind spot: the product isn't where the moment happens...
```

**注意：** 发现部分应如实呈现问题，不要在这里强行插入解决方案。诊断要准确，开方在后面。

#### 6. 解决方案呈现（必须有）
基于前述发现，给出清晰的 **to do next**。

**两种呈现方式：**

**方式 A：在最后一层发现中自然引出方案**
```
Third, the strategic blueprint: five levers emerged from the research.
→ Create a flagship moment (redefine what dairy can be)
→ Unify visual identity (Yili Inside seal across sub-brands)
→ Pivot narrative (from product quality to lifestyle identity)
→ Enable co-creation (let users design, vote, own)
→ Filter partnerships (cultural DNA alignment, not hype-chasing)
```
...
Based on these insights, atypica outlined a three-phase roadmap:
Phase 1: Build identity labels — define "people who choose Yili are..."
Phase 2: Handoff sovereignty — co-design packaging, vote on flavors
Phase 3: Create a flagship moment — tech-forward hero SKU to redefine the brand
```

**核心原则：** 报告的最后部分必须回答"品牌应该做什么"，而不仅仅是"我们发现了什么问题"。
...
- "Trusted brands don't automatically become desired ones — the gap requires identity, not just reliability."

#### 8. CTA 升华（1 句）
诉意化的价值主张：
- "Real simulation. No expensive guesswork"
- "One round of interviews. Multiple rounds of insight"
- "From diagnosis to direction" — 强调 Atypica 不仅发现问题，还指明方向
...
- [ ] **有明确的解决方案部分（to do next — 可以在最后一层发现中，或单独段落）**
- [ ] CTA 升华有诗意和记忆点
...
- [ ] 深入展示了 1-2 个核心功能
- [ ] 说明了功能如何解决具体痛点
- [ ] **展示了从诊断到方案的完整闭环（不仅发现问题，还给出方向）**
- [ ] **有产品界面回扣画面（至少在一个关键环节展示产品界面）**
...
| **只有问题诊断，没有解决方案** | **在脚本后半部分增加"解决方案"段落，或在最后一层发现中引出 to do next** |
| **缺少产品界面回扣** | **增加【产品界面】标注：输入对话框、上传附件、报告页面截图、完整报告 overview** |
...
**参考案例：** Cross-border pet care product validation
...
**参考案例：** Premium automotive brand (female owner insights)
...
- **dairy_brand_usecase_short.md** - Market validation example (short version)
- **dairy_brand_usecase_long.md** - Market validation example (long version)

These examples demonstrate:
- **Template A**: Market validation (testing new product concepts)
- **Template B**: Reusable panel (re-engaging past research participants)
- **Template C**: Comparative validation (verifying traditional research with Atypica)

All examples follow this skill's standards and can serve as high-quality script references.
...
- **v1.4.0** (2026-06-11)
  - ✅ **定位为短版本（1-2min）** — 适合快速传播，聚焦核心价值
  - ✅ 产品界面回扣聚焦在关键环节（推荐输入和输出）
  - ✅ 中间过程可用图形/动效灵活呈现
  - ✅ 检查清单简化：强调"有产品回扣画面"，不限定具体环节

- **v1.3.0** (2026-06-11)
  - ✅ 增加"回扣产品界面"指导 — 输入/处理/输出三个环节必须展示产品界面
  - ✅ 提供【产品界面】标注示例
  - ✅ 更新质量检查清单

- **v1.2.0** (2026-06-11)
  - ✅ **强化实践导向** — 脚本必须包含"解决方案"部分，不能只诊断问题
  - ✅ 明确发现部分可以纯粹展示问题，但最后必须给出 to do next
  - ✅ 更新质量检查清单，增加"解决方案完整性"检查项
...
- **v1.0.0** (2026-06-10)
  - Based on multiple anonymized case study analyses
  - Established narrative golden formula (7 stages)
